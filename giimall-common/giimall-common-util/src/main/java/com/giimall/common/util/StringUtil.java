...
public class StringUtil extends org.apache.commons.lang3.StringUtils {
	 ...
	 * @return the UUID (type String) of this StringUtil object.
	 */
	 ...
	 * Created on 2020/3/3
	 *
	 * @param includes of type String[]
	 * @param targetStr of type String
	 * @return boolean
	 */
	public static boolean exist(String[] includes, String targetStr){
		for (String include : includes) {
			if(targetStr.equals(include)){
				return true;
			}
		}
		return false;
	 ...
	 * 用指定连接符号连接字符
	 *
	 * @param concatSymbol of type CharSequence 连接符号
	 * @param charSequence of type CharSequence 连接字符
	 * @param number       of type int 字符数量
	 * @return String
	 * @author wangLiuJing
	 * Created on 2019/11/5
	 */
	public static String join(CharSequence concatSymbol, CharSequence charSequence, int number) {
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < number; i++) {
			//该步即不会第一位有连接符号，也防止最后一位拼接连接符号！
			if (sb.length() > 0) {
	...
	/**
	 * 重写 String.valueOf 方法，不生成 "null" 字符串
	 *
	 * @param obj
	 * @return
	 */
	 ...
	 *
	 * @param collection of type Collection<String>
	...
	public static boolean containsIgnoreCase(Collection<String> collection, String targetString){
		for (String o : collection) {
			if(targetString.equalsIgnoreCase(o)){
				return true;
			}
		}
		return false;
